...
export default class TyrCommands extends EventEmitter {
    ...
    private _defaultLanguage = 'deutsch'
    ...
    private async setUp(client: Client, options?: Options) {
        ...
        if (mongoUri) {
            ...
            if (showWarns) {
                console.warn(
                    'TyrCommands > No MongoDb connection Uri provided. Some features might not work!'
                )
            }
        ...
        if (
            ...
            throw new Error(
                "TyrCommands > The 'commands' directory must be an absolute path. This can be done by using the 'path' module"
            )
        }
        ...
        if (
            ...
            throw new Error(
                "TyrCommands > The 'features' directory must be an absolute path. This can be done by using the 'path' module"
            )
        }
    ...
    public setMongoPath(mongoPath: string | undefined): TyrCommands {
        console.warn(
            'TyrCommands > .setMongoPath() not longer works as expected. Please pass in your mongo URI as a "mongoUri" property using the options object.'
        )
        return this
    }
    ...
    public setTagPeople(targetPeople: boolean): TyrCommands {
