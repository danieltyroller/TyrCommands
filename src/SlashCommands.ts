...
type ApplicationCommandOption = Exclude<ApplicationCommandOptionData, ApplicationCommandSubCommandData | ApplicationCommandSubGroupData | ApplicationCommandNonOptions>[]
...
class SlashCommands {
   ...
   private async setUp(listen: boolean, typeScript = false) {
      ...
      const replyFromCheck = async (reply: string | MessageEmbed | MessageEmbed[], interaction: CommandInteraction) => {
         if (!reply) {
   ...
   public async create(name: string, description: string, options: ApplicationCommandOption, guildId?: string): Promise<ApplicationCommand<{}> | undefined> {
      let commands
