...
export default class CommandHandler {
  ...
  public async registerCommand(
    ...
    if (testOnly && !instance.testServers.length) {
      console.warn(
        `TyrCommands > Command "${names[0]}" has "testOnly" set to true, but no test servers are defined.`
      )
    }
    ...
    if (slash && !(builtIn && !instance.isDBConnected())) {
      ...
      if (minArgs !== undefined && !expectedArgs) {
        throw new Error(
          `TyrCommands > Command "${names[0]}" has "minArgs" property defined without "expectedArgs" property as a slash command.`
        )
      }
      ...
      if (options.length) {
        for (const key in options) {
          ...
          if (lowerCase.match(/\s/g)) {
            ...
            console.log(
              `TyrCommands > Command "${names[0]}" has an option of "${name}" with a white space in it. It is a best practice for option names to only be one word. TyrCommands will modify this for you.`
            )
          }
