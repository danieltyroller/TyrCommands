...
const sendHelpMenu = (message: Message, instance: TyrCommands) => {
...
module.exports = {
  ...
  init: (client: Client, instance: TyrCommands) => {
  ...
  callback: (options: ICallbackObject) => {
    ...
    if (guild && !guild.me?.permissions.has('SEND_MESSAGES')) {
      console.warn(
        `TyrCommands > Could not send message due to no permissions in channel for ${guild.name}`
      )
      return
    }
