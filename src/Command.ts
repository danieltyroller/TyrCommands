...
class Command {
    ...
    private _requiredRoles?: Map<String, string[]> = new Map() // <Guild, RolesIDs[]
    ...
    public verifyCooldown(cooldown: string, type: string) {
        if (typeof cooldown !== 'string') {
            throw new Error(
                `Invalid ${type} formate! Must be a string, example: "10s" "5m" etc.`
            )
        }
        ...
        if (results.length !== 2) {
            throw new Error(
                `Invalid ${type} formate! Please provide "<Duration><Type>", examples: "10s" "5m" etc.`
            )
        }
        ...
        if (
            ...
            throw new Error(
                `Invalid ${type} formate! Please provide 's', 'm', 'h', or 'd' for seconds, minutes, hours, or days respectively.`
            )
        }
        ...
        if (
            ...
            throw new Error(
                `Invalid ${type} formate! The minimum duration for a global cooldown is 1m.`
            )
        }
    ...
    public verifyDatabaseCooldowns() {
        if (
            ...
            if (!this.instance.isDBConnected()) {
                console.warn(
                    `TyrCommands > A database connection is STRONGLY RECOMMENDED for cooldowns of 5 minutes or more.`
                )
            }
        }
    }
