...
class SlashCommands {
    ...
    async create(name, description, options, guildId) {
        if (!this._instance.isDBConnected()) {
            console.log(`TyrCommands > Cannot register slash command "${name}" without a database connection.`);
            return;
        }
        // @ts-ignore
    ...
    async delete(commandId, guildId) {
        console.log(`TyrCommands > Deleting${guildId ? ' guild' : ''} slash command "${name}"`);
        const commands = this.getCommands(guildId);
        if (commands) {
