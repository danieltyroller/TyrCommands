...
class CommandHandler {
    ...
    constructor(instance, client, dir, disabledDefaultCommands) {
        ...
        if (dir) {
            ...
            client.on('messageCreate', (message) => {
                ...
                const tag1 = `<@!${client.user.id}>`;
                const tag2 = `<@${client.user.id}>`;

                if (content.toLowerCase() === tag1 || content.toLowerCase() === tag2) {
                    content = ' help'
                } else if (content.toLowerCase().startsWith(tag1)) {
                    content = content.substring(tag1.length)
                } else if (content.toLowerCase().startsWith(tag2)) {
                    content = content.substring(tag2.length)
                ...
                const owner = instance.botOwner.includes(user.id);
                const admin = instance.botAdmin.includes(user.id);
                ...
                if (guild && member) {
                    if(command._ownerOnly && !owner) {
                        message.reply(instance.messageHandler.get(guild, 'BOT_OWNERS_ONLY'));
                        return;
                    }
                ...
                // Check for cooldowns
                if ((cooldown || globalCooldown) && user && !owner) {
                ...
                if (guild) {
                    ...
                    if (channels &&
                        channels.length &&
                        !channels.includes(message.channel.id) && !owner) {
                        let channelList = '';
    ...
    async registerCommand(instance, client, file, fileName) {
        ...
        if (!name && (!names || names.length === 0)) {
            throw new Error(`TyrCommands > Command located at "${file}" does not have a name, commands array, or aliases array set. Please set at lease one property to specify the command name.`);
        }
        if (typeof names === 'string') {
            names = [names];
        }
        if (typeof name !== 'string') {
            throw new Error(`TyrCommands > Command located at "${file}" does not have a string as a name.`);
        }
        ...
        if (requiredPermissions || permissions) {
            for (const perm of requiredPermissions || permissions) {
                if (!permissions_1.permissionList.includes(perm)) {
                    throw new Error(`TyrCommands > Command located at "${file}" has an invalid permission node: "${perm}". Permissions must be all upper case and be one of the following: "${[
