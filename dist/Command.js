...
class Command {
    ...
    constructor(instance, client, names, callback, error, { category, minArgs, maxArgs, syntaxError, expectedArgs, description, requiredPermissions, permissions, cooldown, globalCooldown, ownerOnly = false, hidden = false, guildOnly = false, allowAdmin = false, testOnly = false, slash = false, }) {
        ...
        if (requiredPermissions && permissions) {
            throw new Error(`TyrCommands > Command "${names[0]}" has both requiredPermissions and permissions fields. These are interchangeable but only one should be provided.`);
        }
        if (this.cooldown) {
        ...
        if (this._minArgs < 0) {
            throw new Error(`TyrCommands > Command "${names[0]}" has a minimum argument count less than 0!`);
        }
        if (this._maxArgs < -1) {
            throw new Error(`TyrCommands > Command "${names[0]}" has a maximum argument count less than -1!`);
        }
        if (this._maxArgs !== -1 && this._maxArgs < this._minArgs) {
            throw new Error(`TyrCommands > Command "${names[0]}" has a maximum argument count less than it's minimum argument count!`);
        }
    }
    ...
    verifyCooldown(cooldown, type) {
        if (typeof cooldown !== 'string') {
            throw new Error(`TyrCommands > Invalid ${type} format! Must be a string, examples: "10s" "5m" etc.`);
        }
        const results = cooldown.match(/[a-z]+|[^a-z]+/gi) || [];
        if (results.length !== 2) {
            throw new Error(`TyrCommands > Invalid ${type} format! Please provide "<Duration><Type>", examples: "10s" "5m" etc.`);
        }
        this._cooldownDuration = +results[0];
        if (isNaN(this._cooldownDuration)) {
            throw new Error(`TyrCommands > Invalid ${type} format! Number is invalid.`);
        }
        ...
        if (this._cooldownChar !== 's' &&
            ...
            throw new Error(`TyrCommands > Invalid ${type} format! Unknown type. Please provide 's', 'm', 'h', or 'd' for seconds, minutes, hours, or days respectively.`);
        }
        if (type === 'global cooldown' &&
            this._cooldownChar === 's' &&
            this._cooldownDuration < 60) {
            throw new Error(`TyrCommands > Invalid ${type} format! The minimum duration for a global cooldown is 1m.`);
        }
        if (this._cooldownDuration < 1) {
            throw new Error(`TyrCommands > Invalid ${type} format! Durations must be at least 1.`);
        }
        if ((this._cooldownChar === 's' || this._cooldownChar === 'm') &&
            this._cooldownDuration > 60) {
            throw new Error(`TyrCommands > Invalid ${type} format! Second or minute durations cannot exceed 60.`);
        }
        if (this._cooldownChar === 'h' && this._cooldownDuration > 24) {
            throw new Error(`TyrCommands > Invalid ${type} format! Hour durations cannot exceed 24.`);
        }
        if (this._cooldownChar === 'd' && this._cooldownDuration > 365) {
            throw new Error(`TyrCommands > Invalid ${type} format! Day durations cannot exceed 365.`);
        }
    }
    get hidden() {
        return this._hidden;
